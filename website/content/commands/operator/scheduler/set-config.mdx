...
- `-preempt-greedy` - Specifies whether greedy preemption is enabled. When
  set to true, allocs whose job has `meta.greedy="true"` are preemptible by
  any non-greedy alloc that needs their resources, regardless of priority
  delta and independently of the other `-preempt-*-scheduler` flags. Only
  fires for service and batch jobs. Must be one of `[true|false]`.

- `-gpu-reserved-cpu-cores` - Specifies the CPU cores to reserve for each
  healthy unallocated GPU on a node. CPU-only jobs are not placed if they would
  consume this reserved CPU capacity. Set to `0` to disable CPU reservation.

- `-gpu-reserved-memory-mb` - Specifies the memory, in MB, to reserve for each
  healthy unallocated GPU on a node. CPU-only jobs are not placed if they would
  consume this reserved memory capacity. Set to `0` to disable memory
  reservation.
...
Reserve one CPU core and 16 GiB of memory for each healthy unallocated GPU on
GPU nodes:

```shell-session
$ nomad operator scheduler set-config -gpu-reserved-cpu-cores=1 -gpu-reserved-memory-mb=16384
Scheduler configuration updated!
```
