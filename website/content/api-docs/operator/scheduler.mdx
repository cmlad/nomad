...
- `SchedulerConfig` `(SchedulerConfig)` - The returned `SchedulerConfig` object has configuration
  ...
  - `GPUResourceReservation` `(GPUResourceReservation)` - Specifies CPU and
    memory capacity to reserve for each healthy unallocated GPU on a node.
    CPU-only task groups are not placed on a GPU node if the placement would
    leave less than this reservation for the remaining free GPUs. GPU-using task
    groups may consume this reserved capacity if they otherwise fit.

    - `CPUCores` `(int: 0)` - CPU cores to reserve per healthy unallocated GPU.
      Nomad converts cores to CPU shares for each node by using the node's
      available CPU shares and usable core count.

    - `MemoryMB` `(int: 0)` - Memory, in MB, to reserve per healthy unallocated
      GPU.
...
- `GPUResourceReservation` `(GPUResourceReservation)` - Specifies CPU and memory
  capacity to reserve for each healthy unallocated GPU on a node. CPU-only task
  groups are not placed on a GPU node if the placement would leave less than
  this reservation for the remaining free GPUs. Omit the object or set both
  fields to `0` to disable the policy.
