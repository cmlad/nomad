...
- `Operand` - Specifies the test to be performed on the two targets. It takes on the
  ...
  - `missing_or_contains_any` - Matches when `LTarget` cannot be resolved, or
    when its comma-separated value contains any value from the comma-separated
    `RTarget`. If `LTarget` is present, `RTarget` must resolve to a string and at
    least one value must match. `RTarget` is required when registering the job.
...
- `Operand` - Specifies the test to be performed on the two targets. It takes on the
  ...
  - `missing_or_contains_any` - Matches when `LTarget` cannot be resolved, or
    when its comma-separated value contains any value from the comma-separated
    `RTarget`. Matching nodes receive the configured affinity score. If
    `LTarget` is present, `RTarget` must resolve to a string and at least one
    value must match. `RTarget` is required when registering the job.
