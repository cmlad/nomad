...
- `"missing_or_contains_any"` - Matches when the attribute is not present, or
  when its comma-separated value contains any element from the comma-separated
  `value`. Matching nodes receive the configured affinity score; present
  attributes with no matching element do not. The `value` field is required.

  ```hcl
  affinity {
    attribute = "${meta.workload-types}"
    operator  = "missing_or_contains_any"
    value     = "service,batch"
    weight    = 50
  }
  ```
