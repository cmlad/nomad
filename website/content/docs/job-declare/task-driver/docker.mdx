...
- `checkpoint_restore` - (Optional) Restore the container process from a Docker
  checkpoint when starting the task. The Docker daemon on the client must support
  checkpoint/restore, and any `checkpoint_dir` value is interpreted as a host path
  in the Docker daemon's filesystem namespace.
  ...
  config {
    ...
    checkpoint_restore {
      ...
      checkpoint_dir = "/var/lib/docker-checkpoints/redis"
    }
  }
  ```

  The `checkpoint_restore` block supports:

  - `checkpoint_id` - (Required) The checkpoint name to pass to Docker.
  - `checkpoint_dir` - (Optional) The host directory containing the checkpoint.
